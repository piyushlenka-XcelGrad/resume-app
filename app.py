...
def extract_name_from_filename(filename: str) -> str:
    """Format: Piyush_Raj_Lenka.pdf → Piyush Raj Lenka"""
...
def extract_education(text: str) -> str:
    ...
    # Look in education section first
    edu_start = -1
...
def extract_location(text: str) -> str:
    ...
    # Strategy 3: Known Indian cities (longest match first to avoid partial matches)
    cities = [
...
def extract_total_experience(text: str) -> float:
    ...
    # Direct mention (prefer explicit statement)
    for pat in [
    ...
    # Calculate from date ranges, skipping internship entries
    entries = re.split(r'\n\s*\n+', text)
    total_months = 0
...
def extract_companies(text: str) -> List[str]:
    """Extract company/employer names from the resume experience section."""
    if not text:
        return []
    ...
    # Strategy 1: Lines containing company suffixes
    suffix = (
    ...
    # Strategy 3: Pipe-separated "Company | Role | Dates"
    ...
    # Strategy 4: "at <Company>" context in experience
    for m in re.finditer(
        rf'\bat\s+([A-Z][A-Za-z0-9\s&]+(?:{suffix}))',
        search_text
    ):
        c = re.sub(r'\s+', ' ', m.group(1).strip())
        if 4 < len(c) < 75:
...
def process_resume(file_bytes: bytes, filename: str) -> Optional[Dict]:
    ...
    return {
        ...
        '_full_text':         full_text.lower(),   # private — used for search only
    }
...
def search_resumes(
    ...
    """
    Split resumes into (matching, non_matching) based on search terms.
    Each term is checked as a whole-word, case-insensitive match
    ...
    for resume in all_data:
        ...
        for term in clean_terms:
            # Whole-word search (handles multi-word terms gracefully too)
        ...
        # Build display row (strip internal _full_text field)
...
st.markdown(
    "Upload resumes → auto-extract key info → "
    "**dynamically search** by industry, vertical, company name, skill, or any keyword"
)

# ── Reset button (only when data is loaded) ───────────────────────
...
if not st.session_state.processing_done:
    ...
    if st.button("🚀 Process All Resumes", type="primary", disabled=not uploaded_files):
        all_data: List[Dict] = []
        failed: List[str]   = []
        prog   = st.progress(0)
        status = st.empty()
        ...
        st.success(f"✅ Processed **{len(all_data)}** out of {len(uploaded_files)} resumes!")
        st.rerun()
...
# ════════════════════════════════════════════════════════════════════
    ...
    with col_input:
        search_input: str = st.text_input(
            ...
            placeholder=(
                "e.g.,  IT, Pharma, Tata Consultancy, FMCG, Sales, Bangalore, "
                "Python, 5 years, MBA"
            ),
            help=(
                "Type any terms you want to find — industry names, company names, "
                "job titles, skills, cities, or any keyword. "
                "Matches are checked across the full resume text."
            )
        )
    ...
    with col_mode:
        ...
        match_mode: str = st.radio(
            ...
            index=0,
            help=(
                "**OR** — resume is shown if *at least one* term is found.\n\n"
                "**AND** — resume is shown only if *every* term is found."
            )
        )
    ...
    search_clicked = st.button(
        "🔍 Search Resumes",
        type="primary",
        disabled=not search_input.strip()
    )
    ...
    if st.session_state.searched:
        ...
        if matching:
            st.dataframe(
                ...
                height=min(400, 80 + len(matching) * 38)
            )
            ...
            st.info(
                "No resumes matched the given terms. "
                "Try different keywords or switch to **Any (OR)** mode."
            )
        ...
        if non_matching:
            with st.expander(f"Show {len(non_matching)} non-matching resumes", expanded=False):
                st.dataframe(
                    pd.DataFrame(non_matching),
                    use_container_width=True,
                    height=min(400, 80 + len(non_matching) * 38)
                )
                st.download_button(
        ...
        # ── Download All ──────────────────────────────────────────────
        st.download_button(
            "📥 Download All Resumes (Excel — matching + non-matching)",
